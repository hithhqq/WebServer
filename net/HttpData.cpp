    ...
    if (pos < 0) {
        ...
        if (_pos < 0)
            ...
            if (_pos - pos > 1) {
                ...
                if (__pos >= 0) {
                    size_t ___pos = fileName_.find(' ', __pos);
                    query_ = fileName_.substr(__pos + 1, ___pos);
...
AnalysisState HttpData::analysisRequest()
{
    ...
    else if (method_ == METHOD_GET || method_ == METHOD_HEAD)
    {
        ...
        if (fileName_ == "login")
        {
            size_t pos = query_.find('&');
            std::string username = query_.substr(0, pos);
            std::string passwd = query_.substr(pos + 1);
            LOG << query_;
            ConnectionPool *pool = ConnectionPool::getConnectionPool();
            std::shared_ptr<MysqlConn> database = pool->getConnection();
            std::string sql = "select userpasswd from users where username='" + username + "'";
            std::string userpasswd = database->query(sql);
            if (userpasswd == passwd)
            {
                fileName_ = "success.txt"; 
            }
            else
            {
                fileName_ = "fail.txt";
        ...
        if (fileName_ == "applicate")
        {
            size_t pos = query_.find('&');
            std::string username = query_.substr(0, pos);
            int money = atoi(query_.substr(pos+1).c_str());
            std::cout << money << "\n";
            Method data;
            res = data.apply(username, money);
            if(res != -1)
            {
                fileName_ = "success.txt";
            }
